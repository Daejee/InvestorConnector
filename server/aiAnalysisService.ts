    ...
    try {
      ...
      if (filePath && filePath.startsWith("/objects/")) {
        try {
          ...
          // If PDF text is empty or too short, it might be extraction failed
          if (!pdfText || pdfText.trim().length < 100) {
            console.log("PDF 텍스트 추출 실패 또는 내용 부족, 샘플 텍스트 사용");
            // Use sample text for testing - this should be removed in production
            pdfText = `확실한 실적 바닥 
