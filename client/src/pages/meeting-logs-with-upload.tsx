...
export default function Meetings() {
  ...
  // Meeting minutes upload handlers for ObjectUploader
  ...
  const handleMinutesUploadComplete = async (result: UploadResult<Record<string, unknown>, Record<string, unknown>>) => {
    ...
    if (result.successful && result.successful.length > 0) {
      ...
      try {
        const response = await fetch(`/api/meetings/${uploadingMeetingId}/minutes`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    <div>
      ...
      <Dialog open={showUploadModal} onOpenChange={setShowUploadModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Upload Meeting Minutes / 회의록 업로드</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Upload meeting minutes documents (PDF, DOC, DOCX, TXT) / 
              회의록 문서를 업로드하세요 (PDF, DOC, DOCX, TXT)
            </p>
            ...
            >
              <div className="flex items-center justify-center space-x-2">
                ...
                <span>{uploadingMinutes ? "Uploading... / 업로드 중..." : "Select File / 파일 선택"}</span>
              </div>
            </ObjectUploader>
