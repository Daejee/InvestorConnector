...
export const insertInvestorSchema = createInsertSchema(investors).omit({
  ...
  managedFundAum: z.union([z.number(), z.string()]).optional().nullable(),
  numberOfManagedFunds: z.union([z.number(), z.string()]).optional().nullable(),
});
...
export const analystReports = pgTable('analyst_reports', {
  ...
  fileSize: integer('file_size').notNull(),
  fileType: text('file_type').notNull(),
  description: text('description'), // 우려사항 (기존 필드 재사용)
  targetPrice: text('target_price'), // 목표주가 입력 필드
  contentText: text('content_text'), // 긍정적 요인 (기존 필드 재사용)
